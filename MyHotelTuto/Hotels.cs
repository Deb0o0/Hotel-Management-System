...
namespace MyHotelTuto
{
    public partial class Hotels : Form
    {
        ...
        SqlConnection Con = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\utd\Documents\HotelDbase.mdf;Integrated Security=True;Connect Timeout=30");
        ...
        {
            if (HnameTb.Text == "" || HphoneTb.Text == "" || HaddressTb.Text == "")
            {
            ...
            else
            {
                ...
                {
                    Con.Open();
                    SqlCommand cmd = new SqlCommand("update HotelTbl set HName=@HN,HPhone=@HP,HAddress=@HA where HNum=@HKey", Con);
                    cmd.Parameters.AddWithValue("@HN", HnameTb.Text);
                    cmd.Parameters.AddWithValue("@HP", HphoneTb.Text);
                    cmd.Parameters.AddWithValue("@HA", HaddressTb.Text);
        ...
        private void InsertHotel()
        {
            ...
            else
            {
                ...
                {
                    Con.Open();
                    SqlCommand cmd = new SqlCommand("insert into HotelTbl(HName,HPhone,HAddress) values(@HN,@HP,@HA)", Con);
                    cmd.Parameters.AddWithValue("@HN", HnameTb.Text);
                    cmd.Parameters.AddWithValue("@HP", HphoneTb.Text);
                    cmd.Parameters.AddWithValue("@HA", HaddressTb.Text);
                    cmd.ExecuteNonQuery();
        ...
        private void HotelsDGV_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            HaddressTb.Text = HotelsDGV.SelectedRows[0].Cells[3].Value.ToString();
            if (HnameTb.Text == "")
            {
                Key = 0;
            }
